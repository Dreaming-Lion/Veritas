...
DEFAULT_MAX_SENTENCES = int(os.getenv("SUMMARY_MAX_SENTENCES", "2"))
...
try:
    ...
    log.warning("[summary] sklearn missing: %s (fallback lead)", e)
...
def _is_junk_sentence(s: str) -> bool:
    if len(s) <= 5:
        return True
    junk_kw = ["무단 전재", "All rights reserved", "©", "이메일 :", "카카오톡 :", "사진=", "구독", "광고"]
    return any(k in s for k in junk_kw)

def preclean(text: str) -> str:
    """
    요약 전 본문에서 보일러플레이트/캡션/저작권/연락처 등을 제거.
    개행/공백도 정돈.
    """
    ...
    text = "\n".join(kept)
    # 기자 서명 제거
    text = re.sub(r"[가-힣A-Za-z·\.\-]+ 기자\s*[A-Za-z0-9_.+-]+@[A-Za-z0-9_.+-]+", " ", text)
    # 공백 정리
    text = re.sub(r"[ \t\u00A0]+", " ", text)
    text = re.sub(r"\n{3,}", "\n\n", text)
    return text.strip()
...
def split_sentences_ko(text: str) -> List[str]:
    ...
    sents = [re.sub(r"\s*\((?:사진|영상|자료|뉴스1|연합뉴스|무단 전재.*?|All rights reserved).*?\)\s*$", "", s).strip()
             for s in sents]
    sents = [s for s in sents if len(s) >= 6 and not _is_junk_sentence(s)]
    return sents

# 요약기
def _hf_summarize(text: str, max_sentences: int, max_chars: Optional[int]) -> str:
    # 문단 chunking
    outs, buf, total, chunks = [], [], 0, []
    ...
    # 문장 수만 제한(글자수는 기본 OFF)
    picked = sents[:max_sentences] if max_sentences and max_sentences > 0 else sents
    ...
    if max_chars is not None and max_chars > 0 and len(out) > max_chars:
        ...
        for s in picked:
            if total + len(s) <= max_chars:
                clipped.append(s); total += len(s) + 1
            else:
                break
        out = " ".join(clipped).strip()
    return out

def _lexrank_summarize(text: str, max_sentences: int, max_chars: Optional[int]) -> str:
    sents = split_sentences_ko(text)
    if not sents:
        return ""

    if not _has_sklearn or len(sents) == 1:
        picked = sents[:max_sentences] if max_sentences and max_sentences > 0 else sents
        out = " ".join(picked).strip()
        if max_chars is not None and max_chars > 0 and len(out) > max_chars:
            clipped, total = [], 0
    ...
    keep = sents[:80]  
    vec = TfidfVectorizer(analyzer="char", ngram_range=(3, 5), min_df=1)  # type: ignore
    X = vec.fit_transform(keep)
    sim = cosine_similarity(X)  # type: ignore
    import numpy as np  # type: ignore
    np.fill_diagonal(sim, 0.0)
    scores = sim.sum(axis=1)
    idx = scores.argsort()[::-1][:min(max_sentences or 3, 3)]
...
def summarize(text: str,
              ...
              max_chars: Optional[int] = DEFAULT_MAX_CHARS) -> str:
    raw = preclean(text) 
    if not raw:
        return ""
    if _hf:
        try:
            return _hf_summarize(raw, max_sentences or 0, max_chars)
        except Exception as e:
            log.warning("[summary] HF failed: %s; fallback to LexRank", e)
    return _lexrank_summarize(raw, max_sentences or 0, max_chars)

# -------------------- 핵심 로직 --------------------
def _update_summary_for_ids(ids: List[int],
                            *,
                            force: bool = False,
                            max_sentences: Optional[int] = DEFAULT_MAX_SENTENCES,
                            max_chars: Optional[int] = DEFAULT_MAX_CHARS) -> Dict[str, Any]:
    if not ids:
    ...
    try:
        if not acquire_lock(cur):
            return {"skipped": True, "reason": "locked", "updated": 0, "total": 0}
...
def _update_summary_missing(limit: int = 200,
                            *,
                            max_sentences: Optional[int] = DEFAULT_MAX_SENTENCES,
                            max_chars: Optional[int] = DEFAULT_MAX_CHARS) -> Dict[str, Any]:
    conn = get_conn(); cur = conn.cursor()
...
# 크롤링 이후 내부 호출용
def run_summary_after_crawl(ids: Optional[List[int]] = None,
                            *,
                            force: bool = False,
                            limit: int = 200,
                            max_sentences: Optional[int] = DEFAULT_MAX_SENTENCES,
    ...
    init_db()
    if ids:
        return _update_summary_for_ids(ids, force=force,
                                       max_sentences=max_sentences, max_chars=max_chars)
    return _update_summary_missing(limit=limit,
                                   max_sentences=max_sentences, max_chars=max_chars)
...
def webhook_crawl_complete(payload: Dict[str, Any] = Body(...)):
    ...
    max_sentences: Optional[int] = payload.get("max_sentences", DEFAULT_MAX_SENTENCES)
    max_chars_raw = payload.get("max_chars", None)
    max_chars: Optional[int] = int(max_chars_raw) if isinstance(max_chars_raw, int) or (isinstance(max_chars_raw, str) and max_chars_raw.isdigit()) else None

    if ids:
        res = _update_summary_for_ids(ids, force=force,
                                      max_sentences=max_sentences, max_chars=max_chars)
    else:
        res = _update_summary_missing(limit=limit,
                                      max_sentences=max_sentences, max_chars=max_chars)
    return {"ok": True, **res}
